...
- changes:
  - **Bug fixes**
    - #337 The value of command should be `RCPT` when `RCPT first` in the error message.
  ...
  - **MTA modules and error message patterns**
    - #336 #346 Improvement in error message patterns.
      - Update error message patterns in `AuthFailure`, `ContentError`, `NoRelaying`, `Rejected`,
        `UserUnknown`, `SystemError`, `Suspend`, and `rhost/microsoft`.
      - Remove error message patterns already defined in `reason` package from some MTA modules of
        `lhost` package.
    ...
    - #415 Implement `lhost/via-deutschetelekom.go` to decode bounce messages generated by Deutsche
  ...
  - **EXPERIMENTAL Features**
    ...
    - #376 #380 Implement `Bogus` field as an Integer with `-1` as the default value to record the
