...
//   Returns:
//     - (bool): true if the bounce email is an ARF.
//   See:
//     - https://datatracker.ietf.org/doc/html/rfc5965
func isARF(bf *siba.BeforeFact) bool {
	...
	if moji.Aligned(ctype, []string{"report-type=", "feedback-report"}) { return true }
	if strings.Contains(ctype, "multipart/mixed") {
		...
		if strings.Contains(bf.Headers["subject"][0], "complaint about message from ") {
			...
			if moji.ContainsAny(bf.Headers["from"][0], abuse) { return true }
		}
	}
...
func Inquire(bf *siba.BeforeFact) *siba.RisingUnderway {
	...
	if bf == nil || bf.IsEmpty() || isARF(bf) == false { return nil }

	boundaries := [][]byte{
		[]byte("Content-Type: message/rfc822"),
		[]byte("Content-Type: text/rfc822-headers"),
		[]byte("Content-Type: text/rfc822-header"),  // ?
	}
	reportpart := false
	...
	messagebuf := strings.Builder{}; messagebuf.Grow(1024)
	...
	for e := range strings.Lines(emailparts[0]) {
		...
		if readcursor == 0 {
			...
			r := strings.ToLower(e); for _, f := range arfpreface {
				...
				if moji.Aligned(r, f) == false { continue }
				readcursor |= lhost.HereIsDeliveryStatus
				messagebuf.WriteByte(' '); messagebuf.WriteString(e)
				break
			}
			continue
		}
		...
		if reportpart {
			...
			// Source-IP: 192.0.2.1
			if moji.HasPrefixAny(e, []string{"Original-Rcpt-To: ", "Removal-Recipient: "}) {
				...
				if len(v.Recipient) > 0 {
					...
					messagebuf.Reset(); messagebuf.Grow(len(v.Diagnosis))
	...
	if messagebuf.Len() > 0 { dscontents[len(dscontents) - 1].Diagnosis = messagebuf.String() }
	...
	for recipients == 0 {
		...
		if len(bf.Headers["x-apple-unsubscribe"]) > 0 {
			...
			cv := address.S3S4(moji.Select(emailparts[1], "\nTo:", "\n", 0))
