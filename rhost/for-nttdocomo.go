...
func init() {
	...
	ReturnedBy["NTTDOCOMO"] = func(fo *siba.Fact) string {
		...
		if statuscode == "5.0.0" {
			...
			// 550 Unknown user ***@docomo.ne.jp (in reply to end of DATA command)
			// ...
			// Final-Recipient: rfc822; ***@docomo.ne.jp
			...
			// Diagnostic-Code: smtp; 550 Unknown user ***@docomo.ne.jp
			if fo.Command == eb.CeRCPT { return eb.ReUSER }
			if fo.Command == eb.CeDATA { return eb.ReFROM }
		}

		// 1. Rejected by other SMTP commands: AUTH, MAIL, or any SMTP commands except RCPT and DATA.
		// 2. The Status: field is neither 5.0.0 nor values defined in code above
		...
		//   please open an issue at https://github.com/sisimai/go-sisimai/issues page.
		return ""
	}
}
