...
func init() {
	...
	ReturnedBy["YahooInc"] = func(fo *siba.Fact) string {
		...
		messagesof := map[string][]string{
			...
			eb.ReNRFC: []string{"headers are not rfc compliant"},
			...
			eb.ReFROM: []string{ // Rejected
				...
				//
				// However, the same error message is returned even for domains that are considered
				// to have a poor reputation without SPF, DKIM, or DMARC settings, or for other reasons.
