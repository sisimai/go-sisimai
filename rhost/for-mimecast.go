...
func init() {
	...
	ReturnedBy["Mimecast"] = func(fo *siba.Fact) string {
		...
		messagesof := map[string][][2]string{
			eb.ReAUTH: [][2]string{ // AuthFailure
				// - The inbound message has been rejected because the originated IP address isn't 
				//   listed in the published SPF records for the sending domain.
				...
				//   "Inbound SPF" or "Reject on Hard Fail" option disabled. Messages that fail our
				//   SPF checks are subjected to spam and RBL checks, instead of being rejected.
				[2]string{"550", "spf sender invalid - envelope rejected"},

				// - The DKIM key for the outbound message is broken and doesn't match the DNS record
				//   of the registered sender.
				// - Check your organization's DNS record is populated with the right public key as
				//   part of the DNS Authentication Outbound Signing definition. The private key of
				//   the keypair must be populated in the DNS Authentication policy, along with the
				//   domain and selector of that record.
			...
			eb.ReBLOC: [][2]string{ // Blocked
				...
				//   connections.
				// - The sender's mail server must retry the connection. The mail server performing
				//   the connection says the recipient address validation isn't responding.
				...
				// - The sender's IP address is listed in an RBL. The text displayed is specific to
				//   the RBL which lists the sender's IP address.
			...
			eb.ReTTLS: [][2]string{ // FailedSTARTTLS
				...
				// - A secure connection was attempted using ciphers that do not meet the configured
				//   cipher strength.
				// - Delete or change the Secure Receipt or Secure Delivery policy enforcing TLS.
				//   Alternatively, ensure the certificates on the mail server haven't expired. If
				//   using a proxy server, ensure it isn't intercepting the traffic and modifying
				//   encryption parameters.
			...
			eb.ReSIZE: [][2]string{ // EmailTooLarge
				...
				//   and content-encoding can add significantly to the total message size (e.g. a
			...
			eb.ReINET: [][2]string{ // NetworkError
				...
				// - The message has too many "received headers" as it has been forwarded across
				//   multiple hops. Once 25 hops have been reached, the email is rejected.
			...
			eb.Re___1: [][2]string{ // OnHold
				...
				// - Attempts are being made to journal mail that is past the set expiry threshold.
				//   The failure will be replaced by a retry response because the message is marked
				//   for retry if rejected, causing the journal queue to grow.
				// - Check to confirm there are no significant time discrepancies on the mail server.
				//   Discontinue journaling old messages past the expiry threshold.
			...
			eb.ReFROM: [][2]string{ // Rejected
				...
				// - A Block Sender Policy has been applied to reject emails based on the Header From
			...
			eb.ReSAFE: [][2]string{ // SecurityError
				...
				//   with a corresponding Mimecast cloud password. Alternatively, consider sending
				//   the message on SMTP port 25.
			...
			eb.RePROC: [][2]string{ // SystemError
				...
				//   If running a Cisco Firewall, ensure the Mailguard or SMTP Fixup module is disabled.
