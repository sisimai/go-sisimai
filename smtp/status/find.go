...
func Find(logs string, hint string) string {
	...
	ip4address := rfc791.FindIPv4Address(logs)
	for _, e := range ip4address { logs = strings.ReplaceAll(logs, e, "***.***.***.***") }

	esmtperror := []byte(" " + logs + "   "); cw := len(esmtperror)
	lookingfor := make(map[string][]byte, 10)
	indextable := make([]int, 0, 10)
	eestatuses := make([][]byte, 0, 3)
	switch hint[0:1] {
		case "2", "4", "5": eestatuses = append(eestatuses, []byte(hint[0:1] + "."))
		default:            eestatuses = [][]byte{[]byte("5."), []byte("4."), []byte("2.")}
	}
	...
	bu := bytes.Buffer{}; bu.Grow(5)
	for _, e := range indextable {
		// Try to find an SMTP Status Code from the given string
		cu := fmt.Sprintf("%04d", e)
		ci := bytes.Index(esmtperror[e:], lookingfor[cu]); if ci < 0 { continue }
		ci += e
		cx := [8]byte{0, 0, 0, 0, 0, 0, 0, 0}

		if cw > ci     { cx[0] = esmtperror[ci - 1:ci][0]     } // [0] The previous character of the status
		if cw > ci + 3 { cx[1] = esmtperror[ci + 2:ci + 3][0] } // [1] The value of the "Subject", "5.[7].261"
		if cw > ci + 4 { cx[2] = esmtperror[ci + 3:ci + 4][0] } // [2] "." chacater, a separator of the Subject and the Detail
		...
		if bu.Len() > 0 { bu.Reset() }
		bu.Write(lookingfor[cu]); bu.WriteByte(cx[1]); bu.WriteByte('.')

		if cw > ci + 5 { cx[3] = esmtperror[ci + 4:ci + 5][0] } // [3] The 1st digit of the detail
		if cw > ci + 6 { cx[4] = esmtperror[ci + 5:ci + 6][0] } // [4] The 2nd digit of the detail
		if cw > ci + 7 { cx[5] = esmtperror[ci + 6:ci + 7][0] } // [5] The 3rd digit of the detail
		if cw > ci + 8 { cx[6] = esmtperror[ci + 7:ci + 8][0] } // [6] The next character

		if cx[3] < 48 || cx[3] > 57 { continue } // The 1st digit of the detail is not a number
		bu.WriteByte(cx[3])

		if cv := bu.String(); IsAmbiguous(cv) || cv == "4.4.7" {
			// Find another status code except *.0.0, 4.4.7
			anotherone = bu.String(); continue
		}

		// The 2nd digit of the detail is not a number
		if cx[4] < 48 || cx[4] > 57 { statuscode = append(statuscode, bu.String()); continue }
		bu.WriteByte(cx[4]) // The 2nd digit of the detail is a number

		// The 3rd digit of the detail is not a number
		if cx[5] < 48 || cx[5] > 57 { statuscode = append(statuscode, bu.String()); continue }
		bu.WriteByte(cx[5]) // The 3rd digit of the detail is a number

		if cx[6] > 47 && cx[6] < 58 { continue }
		statuscode = append(statuscode, bu.String())
	}
