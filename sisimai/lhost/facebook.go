...
func init() {
	// Decode bounce messages from Facebook: https://www.facebook.com'
	InquireFor["Facebook"] = func(bf *sis.BeforeFact) sis.RisingUnderway {
		// @param    *sis.BeforeFact bf  Message body of a bounce email
		// @return   RisingUnderway      RisingUnderway structure
		if len(bf.Head) == 0 { return sis.RisingUnderway{} }
		if len(bf.Body) == 0 { return sis.RisingUnderway{} }
		if bf.Head["from"][0]    != "Facebook <mailer-daemon@mx.facebook.com>"   { return sis.RisingUnderway{} }
		if bf.Head["subject"][0] != "Sorry, your message could not be delivered" { return sis.RisingUnderway{} }

		indicators := INDICATORS()
		boundaries := []string{"Content-Disposition: inline"}
		startingof := map[string][]string{ "message": []string{"This message was created automatically by Facebook."} };
