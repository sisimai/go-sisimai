...
func TestLooksLikeEmail(t *testing.T) {
	...
	xe := [][]byte{[]byte(""), []byte("\x00nekocat"), []byte("neko\x7fcat"), []byte("neko\xffcat")}
	...
	for _, e := range ae {
		...
		cx++; if LooksLikeEmail(by) == false { t.Errorf("rfc5322.%s(%s) returns false", fn, e) }
	}
	...
	for _, e := range []string{filepath.Join("..", ".gitignore"), filepath.Join("..", "set-of-emails", "mailbox", "size-2")} {
		by, _ := os.ReadFile(e)
		cx++; if LooksLikeEmail(by) == true  { t.Errorf("rfc5322.%s(%s) returns true ", fn, e) }
	}
