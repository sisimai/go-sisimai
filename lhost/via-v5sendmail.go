...
func init() {
	...
	InquireFor["V5sendmail"] = func(bf *siba.BeforeFact) *siba.RisingUnderway {
		// - Sendmail version 5
		...
		for e := range strings.Lines(emailparts[0]) {
			...
			if readcursor == 0 {
				...
				if strings.Contains(e, startingof["message"][0]) { readcursor |= HereIsDeliveryStatus }
				continue
			}
		...
		if recipients == 0 {
			...
			if recipients == 0 {
				...
				if cv := moji.Select(emailparts[1], "\nTo: ", "\n", 0); cv != "" {
					...
					if rfc5322.IsEmailAddress(cv) == false { return nil }
