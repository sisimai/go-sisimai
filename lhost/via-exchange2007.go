...
func init() {
	...
	InquireFor["Exchange2007"] = func(bf *siba.BeforeFact) *siba.RisingUnderway {
		...
		ndrsubject := map[string]string{
			...
			"RESOLVER.ADR.BadPrimary":        eb.RePROC, // 550 5.2.0 RESOLVER.ADR.BadPrimary
		...
		for e := range strings.Lines(emailparts[0]) {
			...
			if strings.IndexByte(e, ' ') < 0 && strings.IndexByte(e, '@') > 1 {
				...
				if moji.HasPrefixAny(e, startingof["rhost"]) {
				...
				} else if x, y := reply.Find(e, ""), status.Find(e, ""); len(x + y) > 0 || strings.Contains(e, "Remote Server ") {
					// #550 5.1.1 RESOLVER.ADR.RecipNotFound; not found ##
		...
		for recipients == 0 {
			...
			//   Subject:        Nyaan?
			if strings.Index(emailparts[0], "Original Message Details") < 0 { break }

			cv := address.S3S4(moji.Select(emailparts[0], "\nRecipient Address: ", "\n", 0))
