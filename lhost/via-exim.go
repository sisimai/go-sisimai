...
func init() {
	...
	InquireFor["Exim"] = func(bf *sis.BeforeFact) sis.RisingUnderway {
		...
		for _, e := range(strings.Split(emailparts[0], "\n")) {
			...
			if cv := ""; ce == true || sisimoji.ContainsAny(e, startingof["alias"]) {
				...
				if sisimoji.ContainsAny(e, startingof["alias"]) {
					...
					cv = address.S3S4(sisimoji.Select(e, "<", ">:", 0)); if cv != "" {
		...
		if recipients > 0 {
			...
			for j, _ := range dscontents {
				...
				e := &(dscontents[j])
				if e.Alias == "" { continue }
				if strings.IndexByte(e.Recipient, '@') < 0 {
			...
			if e.Command == "" {
				...
				if e.Command == "EHLO" || e.Command == "HELO" {
					...
					for r := range messagesof {
						...
						if sisimoji.ContainsAny(e.Diagnosis, messagesof[r]) { e.Reason = r; break }
					}
