...
func init() {
	...
	InquireFor["AmazonSES"] = func(bf *siba.BeforeFact) *siba.RisingUnderway {
		...
		var notdecoded []siba.NotDecoded
		...
		if whatnotify == "" {
			...
			ce := *siba.MakeNotDecoded(jsonerrors.Error(), true); ce.DecodedBy = "AmazonSES"
			notdecoded = append(notdecoded, ce)
			return &siba.RisingUnderway{Errors: notdecoded}
		}
